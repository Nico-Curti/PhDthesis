...
![Synthetic dataset simulation. Comparison of accuracy performances obtained by the DNetPRO algorithm and the `K`-best algorithm. **(b - right)** Performances obtained in function of the number of variables, keeping fixed the number of samples.](https://raw.githubusercontent.com/Nico-Curti/PhDthesis/master/img/features_toy.svg?token=AF4CJXYWNLWYHWIIEEWMFES5VYJ7Y&sanitize=true)
